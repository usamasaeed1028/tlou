...
const bg=type=='total'?'rgba(114,137,218,0.2)':type=='wins'?'rgba(44,185,22,0.2)':'rgba(212,18,18,0.2)'
...
const {setSelectedUser, selectedUser, setIsEditing, isEditing } =useUserManagmentInternalState();
  ...
  className="w-1/3   flex rounded-xl items-center xl:pl-[15px] pl-2 gap-2 xl:gap-4  "
>
  <div
    className={`w-[40px]  aspect-square xl:w-[70px] rounded-[14px] flex items-center justify-center ${type=='total'?'bg-[rgba(114,137,218,0.2)]':type=='wins'?'bg-[rgba(44,185,22,0.2)]':'bg-[rgba(212,18,18,0.2)]'}  `}
  >
  ...
  <input
      ...
      name={type}
      className={`w-20 h-7 xl:text-[24px]  text-base font-bold text-white  bg-transparent pl-2 ${isEditing ? "shadow-inputModification  rounded-md" : ""}`}
      value={selectedUser.games[type]}
      disabled={!isEditing}
    />
    <p className='text-xs xl:text-sm text-[#e7e3fc] font-normal '>Total {type=='total'?'Matches':type=='wins'?'Wins':'Losses'}</p>
  </div>
</div>
}
