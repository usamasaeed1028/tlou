    ...
    <div className="w-full h-full ">
      <div className="w-full h-fit   relative">
        <div
          ...
          className="absolute z-10 rounded-[10px] scrollbar-hide  top-7 left-0 right-0 h-fit  overflow-x-scroll bg-black"
        >
          <div className="w-[1472px] h-fit  font-inter ">
            <div className="w-full h-[78px] items-center flex justify-between pl-[50px] pr-5">
              <h2 className=" font-semibold text-[28px] text-[#cecee0]">
                Penalty History
              </h2>
              ...
              <div className="flex gap-4">
                <div
                  ...
                  className="h-10 w-[246px] rounded-lg  "
                >
                ...
                style={{
                    ...
                    className=" w-[86px] h-[38px]  text-xs rounded-lg  block  p-1.5 bg-[rgba(30,30,30,0.69)]   text-white  "
                ...
                style={{
                 ...
                 className="w-[86px] h-[38px] rounded-lg text-sm font-medium text-[#cecee0]">History</button>

                <button className="w-[86px] h-[38px] bg-[#FFB53B] rounded-lg text-sm font-medium text-white">
                        New
                </button>
              </div>
            </div>
        ...
        <div>
        <table {...getTableProps()}  className=" w-[1472px]">
            <thead  >
            {headerGroups.map((headerGroup)=>{
                return <tr
                 ...
                 className="text-left  text-base font-inter font-medium text-white w-full h-[61px]" {...headerGroup.getHeaderGroupProps()}>
                {headerGroup.headers.map(column=>{
                    
                    if(column.Header=='ACTION'){
                        return <th className="text-left px-10 " {...column.getHeaderProps()}>{column.render('Header')}</th>
                    }
                    return <th className="px-[54px]" {...column.getHeaderProps()}>{column.render('Header')}</th>
                })}
        ...
        {rows.map((row, i) => {
          prepareRow(row)
          return (
            <tr className="h-[65px] relative  text-sm text-[#adadad] font-inter" {...row.getRowProps()}>
              {row.cells.map(cell => {
                    ...
                    if(cell.column.Header == "PENALTY ADDED BY"){
                        ...
                        return <td  className='px-[54px] h-full  '>
                    ...
                    if(cell.column.Header=='STATUS'){
                        ...
                        return <td className='px-[54px] h-full  '>
                                <span className={`px-[34px] py-[7px]  rounded-3xl text-base ${cell.value=='warning'?'text-[#ec9e08] bg-[#EC9E08] bg-opacity-[18%]':'text-[#D41212] bg-[#D41212] bg-opacity-[14%]'}`}>{cell.render("Cell")}</span>
                        </td>
                    }
                    return (
                      <td
