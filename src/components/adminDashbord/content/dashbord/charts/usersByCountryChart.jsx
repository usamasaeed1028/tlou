...
const UsersByCountryChart = () => {
    ...
    return ( <div className='  flex flex-col    h-full mx-auto'>
    <Doughnut data={data} options={options} plugins={[gaugetext]} className='my-auto w-2/3' />
        <div className=' w-2/3 flex  flex-wrap items-center justify-center aspect-foughnutChartNewLgend-24/4 mx-auto '>
                {data.labels.map((item,index)=>{
                  ...
                  <p className='text-[#EAEAFF] 2xl:text-lg lg:text-xs text-[8px]'>{item }</p>
                    </div>
                })}
        </div>
    </div> 
